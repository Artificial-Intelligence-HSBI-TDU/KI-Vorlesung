...
- Die Menge aller gelabelten Daten in **Trainingsset und Testset**
  ...
  - $E_{in}$ bezeichnet den Fehler auf dem Trainingsset, auch
    **in-sample error**.
  - $E_{out}$ bezeichnet den Fehler auf dem gesamten Eingaberaum $X$,
    auch **out-of-sample error**. $E_{out}$ ist der eigentliche
...
- Die Näherung $E_{test}$ sollte möglichst genau sein, damit es als ein
  ...
  - Das Testset sollte genug Daten enthalten. Üblicher Anteil an
    ...
    - Beispiel: Hat man 1000 Beispiele im Testset, wird $E_{test}$ mit
      ...
      $E_{out}$ liegen (für theoretische Grundlagen und Herleitung siehe
      ([Abu-Mostafa, Magdon-Ismail, und Lin 2012,
...
- Der **Validierungsfehler $E_{val}$** kann nun als
  ...
  </div><p>Abbildung 1 - Einsatz der Validierung für das
  ...
  </div><p>Abbildung 2 - Einsatz der Validierung bei der Auswahl der
  entgültigen Hypothese</p></div>
  ...
  </div><p>Abbildung 3 - Lernkurven</p></div>
...
- Das Modell $(\mathcal{H_m},\mathcal{A_m})$ wird $k$ mal trainiert und
  validiert, jedes mal mit unterschiedlichen Trainings- und
  Validierungsmengen:
  ...
  - Bei dem $i$-ten Training werden die Teilmenge $D_i$ für die
    Berechnung des Validierungsfehlers $e_i := E_{val}(h_m^{*(i)})$ und
    die restlichen $k-1$ Teilmengen für das Training verwendet.
  ...
  </div><p>Abbildung 4 - Kreuzvalidierung</p></div>
